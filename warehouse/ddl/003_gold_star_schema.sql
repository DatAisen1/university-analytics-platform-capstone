-- warehouse/ddl/003_gold_star_schema.sql
--
-- Explicit table definitions for the Gold star schema (Task 23/24 --
-- Gold Modeling Fix). Run AFTER 001_create_schemas.sql.
-- pipelines/gold/load_gold_to_postgres.py's replace_table_contents()
-- will use these tables (TRUNCATE+append) once they exist, rather than
-- implicitly creating them via pandas.to_sql.
...
-- Database-enforced version of the "exactly one current row per student"
-- invariant your test suite already checks in Python.
