...
**Regenerated against the fixed `fit_prophet()` (P0-A.1 resolved).** Prior drafts of this note flagged two rounds of staleness: first a grain-migration lag (resolved, and disclosed rather than silently fixed, per this project's convention), then a real bug where `fit_prophet()` called bare `Prophet()` and silently ran Prophet's `'auto'` seasonality heuristic instead of the documented `yearly_seasonality=2, weekly_seasonality=False, daily_seasonality=False` configuration. That bug is now fixed — `fit_prophet()` passes all three explicitly (see its docstring in `train_prophet.py` for exactly how `'auto'` diverged from the intended config on this project's real fold sizes), guarded by a regression test (`test_fit_prophet_passes_explicit_seasonality_config_to_prophet`) that inspects the actual Prophet constructor call rather than only checking that fitting succeeds — and `forecasting/artifacts/evaluation_report.md` has been regenerated against it. The numbers below are current as of that regeneration.

**Real results, post-fix:** Prophet beats the best baseline on **31 of 74** series (42%) overall — `enrollment_count` 31/37 (84%, up from 78% under the old `'auto'`-seasonality bug), `graduation_count` 0/37 (0%, unchanged). The explicit `yearly_seasonality=2` configuration produced a materially better fit than `'auto'` on most `enrollment_count` series: MAE dropped sharply and R² moved from roughly 0.2–0.3 to 0.9+ on the majority of programs, consistent with `'auto'` silently running full order-10 yearly seasonality on the longer training windows and overfitting — exactly the failure mode this section's "Prophet configuration" paragraph below was written to prevent. `graduation_count`'s 0/37 is unchanged by this fix: Prophet's Gaussian continuous-value model still can't out-perform baselines on tiny, right-skewed count series regardless of seasonality config, which is the existing rationale for `count_model` (§9) — not something this fix touches.

> ⚠️ **New, unrelated issue found during this regeneration:** the `count_model_mae` column now contains several implausible values in the hundreds of thousands to hundreds of millions (e.g. `COE-BSCE`: 204,425,790.05; `COED-BPE`: 204,425,789.41) on `graduation_count` series where the real counts are single digits, and two rows share an identical MAE (`847298.55`) across unrelated programs to two decimal places — the signature of a diverging or non-converged Negative Binomial dispersion fit that isn't being caught, not real computed error. This does not affect promotion decisions today (`count_model` still can't win `select_champion_algorithm()` with an MAE that large), so it hasn't silently corrupted any deployed forecast — but it should not be left unexamined. Disclosed here rather than fixed quietly; see §9.2's fail-soft table, which currently has no guard for "NB fit converged but produced a degenerate/blown-up dispersion parameter" the way it already does for outright non-convergence.
...
**⚠️ Known gap in the above table, found in the §5.1 evaluation-report regeneration:** the fallback row only covers a fit that *fails to converge*. It does not currently catch a fit that *converges* but to a degenerate/blown-up dispersion parameter — several `count_model_mae` values in the current `evaluation_report.md` are in the hundreds of thousands to hundreds of millions on series where real counts are single digits, which is that case slipping through. Not yet fixed; tracked as a follow-up, not swept under the rug.
