...
> **⚠️ STALE — pending regeneration, for a different reason than last time.** The grain migration this note previously flagged as pending has actually already happened and is already reflected in the current `forecasting/artifacts/evaluation_report.md` (real program-grain results: `~37 programs × 2 metrics`, `29 of 74 series` beating baseline overall, `0 of 37` for `graduation_count` — the exact measurement `models/forecasting/count_model.py`'s docstring cites as what justified building it). That half of the staleness this note used to describe is resolved; leaving the old wording here was itself a doc-sync bug (a real one, found and left as a deliberate example rather than quietly fixed, per this project's own "disclose it" convention).
>
> **The report is genuinely stale again now, for a new and different reason (P0-A.1):** every number in it was produced by `fit_prophet()` calling bare `Prophet()`, which — despite this section's own "Prophet configuration" paragraph below claiming `yearly_seasonality=2` — actually left seasonality on Prophet's `'auto'` heuristic the entire time. `fit_prophet()` has now been fixed to explicitly pass `yearly_seasonality=2, weekly_seasonality=False, daily_seasonality=False` (see its docstring in `train_prophet.py` for exactly how `'auto'` diverged from that on this project's real fold sizes), backed by a regression test (`test_fit_prophet_passes_explicit_seasonality_config_to_prophet`) that inspects the actual Prophet constructor call rather than only checking that fitting succeeds. **`evaluation_report.md` must be regenerated against the fixed `fit_prophet()` before any number in it is treated as current** — the champion mix, the MAE/R² figures, and the `count_model` gating decision could all shift once Prophet is actually running with the documented configuration instead of Prophet's own heuristic.
