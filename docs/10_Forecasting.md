...
**Regenerated against the fixed `fit_prophet()` (P0-A.1 resolved).** Prior drafts of this note flagged two rounds of staleness: first a grain-migration lag (resolved, and disclosed rather than silently fixed, per this project's convention), then a real bug where `fit_prophet()` called bare `Prophet()` and silently ran Prophet's `'auto'` seasonality heuristic instead of the documented `yearly_seasonality=2, weekly_seasonality=False, daily_seasonality=False` configuration. That bug is now fixed — `fit_prophet()` passes all three explicitly (see its docstring in `train_prophet.py` for exactly how `'auto'` diverged from the intended config on this project's real fold sizes), guarded by a regression test (`test_fit_prophet_passes_explicit_seasonality_config_to_prophet`) that inspects the actual Prophet constructor call rather than only checking that fitting succeeds.

**`forecasting/artifacts/evaluation_report.md` is the source of truth for every number in this section.** It has drifted out of sync with this doc before (this section itself has been corrected at least twice — see the two staleness rounds above and the 2026-09 correction below) — if a number here and a number in that file ever disagree again, the artifact is right and this prose needs updating, not the other way around.

**Real results, as of the 2026-09 regeneration:** Prophet beats the best baseline on **43 of 74** series (58%) overall — `enrollment_count` 27/37 (73%), `graduation_count` 16/37 (43%). Win *rate* alone understates how different these two pictures are, though: among the series where Prophet wins, the median error reduction versus the best baseline is 56% for `enrollment_count` but only 8% for `graduation_count` — even Prophet's graduation-count "wins" are typically a fraction of one student's worth of error, not a meaningful edge. `enrollment_count`'s explicit `yearly_seasonality=2` configuration produces a materially better fit than Prophet's `'auto'` default did on most series: R² lands at 0.8–0.95 on a majority of programs. `graduation_count` remains genuinely hard for Prophet regardless of seasonality config — its Gaussian continuous-value model still struggles against a small, right-skewed count series on the majority of programs — which is the existing rationale for `count_model` (§9).

> ✅ **Resolved:** an earlier regeneration of this report surfaced `count_model_mae` values in the hundreds of thousands to hundreds of millions on series where real counts are single digits (e.g. `COE-BSCE`: 204,425,790.05) — the signature of a diverging/degenerate Negative Binomial dispersion fit slipping through uncaught. `count_model.py`'s extrapolation cap (`extrapolation_cap = max(y) + 2*max(max_jump, 1)`, applied to `mu_hat` before it can propagate) already closed the specific case that produced those numbers; the current `evaluation_report.md`'s largest `count_model_mae` value is 42.57. Two further defensive guards were added as belt-and-suspenders hardening (not because the blow-up above was still reproducible, but because the extrapolation cap's `mu_hat > cap` comparison structurally cannot catch a NaN prediction — `NaN > x` is always `False` in numpy, verified directly rather than assumed): a non-finite `mu_hat` guard falling back to the sample mean, and a non-finite interval-bounds guard collapsing to a degenerate zero-width interval. Both are covered by regression tests (`tests/unit/test_count_model.py::test_non_finite_mu_hat_falls_back_to_sample_mean` and `::test_non_finite_interval_bounds_collapse_to_degenerate_zero_width`) that force each failure mode via direct mocking rather than trying to naturally reproduce a numerically pathological fit.

**Modules:** `models/forecasting/metrics.py`, `models/forecasting/baselines.py`, `models/forecasting/train_prophet.py`, run via `python -m models.forecasting.train_prophet`.

**Prophet configuration, and why:** `yearly_seasonality=2` (a low Fourier order, vs. Prophet's default 10) — with as few as 2–3 training points in the earliest walk-forward fold under the 6-semester model, a high-order seasonal fit would overfit even more readily than it would have under the old 8-semester model. `weekly_seasonality`/`daily_seasonality` disabled outright, since this is semester-grain data. **This is now actually enforced in code** (`fit_prophet()` passes all three explicitly) as of the P0-A.1 fix above — previously this paragraph described the intended configuration, not the one the code produced.

**Testing:** `tests/unit/test_forecasting_metrics.py` and `tests/unit/test_forecasting_baselines.py` keep their structure; `tests/unit/test_train_prophet.py` now includes `test_fit_prophet_passes_explicit_seasonality_config_to_prophet` (P0-A.2) guarding the Prophet constructor call itself, in addition to the existing fold-count/grain regression coverage.
...
**Interval provenance (migration `0018_forecast_interval_calibration.py`):** `yhat_lower`/`yhat_upper` do not all mean the same thing — Prophet's interval may be a genuinely MCMC-sampled posterior or a disclosed MAP-only approximation (this project's real evaluation shows the latter is currently the norm: 0 of 222 walk-forward folds achieved genuine MCMC calibration, per `evaluation_report.md`'s "Interval Calibration" column; the underlying reasoning — divergent-transition diagnostics, `MCMC_CALIBRATION_ENABLED = False` — lives in `train_prophet.py`'s module docstring and the `IntervalCalibration` dataclass, not yet mirrored into this doc as its own section), `count_model`'s is a real Poisson/Negative-Binomial quantile, and a baseline's is a deliberately degenerate zero-width band. Every `forecast_grain = 'program'` row now carries `interval_calibration_method` (one of `bayesian_mcmc`, `map_disclosed`, `count_quantile`, `degenerate_zero_width` — see `models.forecasting.deploy_forecast._interval_calibration_for`) and an optional `interval_calibration_note` explaining the specific reason or model detail. Before this migration, this distinction was only ever logged to console (`deploy_forecast.py`) and never reached the table itself — a dashboard or stakeholder query reading `gold.fact_forecast` directly had no way to tell a calibrated interval from an approximation. `college`/`campus` rollup rows leave both columns `NULL` deliberately: a rollup sums several already-promoted program-level forecasts that may carry *different* calibration methods, and collapsing that into one scalar value honestly needs its own design, not a mechanical extension of this fix — tracked as an open follow-up, not silently left inconsistent.

> ✅ **Bug found and fixed within hours of shipping, migration `0019_widen_forecast_interval_calibration_note.py`:** `interval_calibration_note` originally shipped as `VARCHAR(256)`, sized without measuring the real strings it has to hold — `IntervalCalibration.reason` for the project-wide MCMC-disabled disclosure (§9.1/train_prophet.py's `MCMC_CALIBRATION_ENABLED = False` path, the *default*, not a rare edge case) is 389 characters. Every forecast write for a Prophet champion hit `psycopg2.errors.StringDataRightTruncation` at the Forecast Deployment pipeline stage, caught by a real `dagster job execute` run, not by any test in this repo — a genuine test-coverage gap this note doesn't paper over. Fixed by widening the column to `TEXT` in a new migration rather than rewriting `0018` in place, since `0018` had already been applied to a real database by the time the bug surfaced; see `0019`'s own module docstring for why editing an already-applied revision's file would have been the wrong fix even though the error only ever showed up downstream, at the INSERT, not at migration time.
...
Section 8 above quantified *how misleading* a `graduation_count` rejection looks under MAPE. Section 9.1 answers a different question: setting MAPE aside entirely, is Prophet's *absolute* (MAE) performance on `graduation_count` actually fine, or actually bad? A real walk-forward run (`forecasting/artifacts/evaluation_report.md`) answered this directly: Prophet beats its baseline on 16 of 37 (43%) `graduation_count` series — compared to 27 of 37 (73%) for `enrollment_count`, which is not touched by anything in this section. Beyond the raw rate, the *size* of graduation_count's wins is small too: a median 8% error reduction versus baseline among the series Prophet does win, against 56% for enrollment_count (§5.1) — so even "beats baseline" here is typically a fraction of one student's worth of improvement, not a meaningful edge.

The losses on this metric aren't one uniform failure mode; they split into (at least) two genuinely different situations that Section 8's reconciliation function doesn't distinguish, because it wasn't built to:
...
2. **Genuine model-mismatch, on the nonzero subset.** Here Prophet isn't marginally losing — e.g. `COA-CERT-BUT` (Prophet MAE 2.88 vs. best baseline 1.36, R² −5.4) and `CMBT-BSHM` (Prophet MAE 11.97 vs. 10.00, R² −4.4). An R² this far below zero means Prophet's fitted curve does considerably *worse* than just guessing the training mean — the specific failure signature of fitting a Gaussian-noise, continuous-valued trend+seasonality model to a small, non-negative, integer-valued, right-skewed count series. This is the actual gap a count-respecting model can close. (One caution the §5.1 R²-instability note already raises: several of the *most* negative R² values in the current report, e.g. `COED-BSNED` at −54.7, actually still count as Prophet beating its baseline — both models were bad on that fold, Prophet merely less bad — which is exactly why this section leads with two clean, non-tied losing examples rather than the single worst R² in the table.)
...
✅ **Resolved** (previously flagged here as a known gap): the fallback row above only ever covered a fit that *fails to converge*. `count_model.py`'s extrapolation cap now also catches a fit that *converges* but to a degenerate/blown-up dispersion parameter — see §5.1's "Resolved" note for the full history and the two further non-finite guards added as additional hardening. The current `evaluation_report.md`'s largest `count_model_mae` value is 42.57, not the hundreds-of-thousands-to-millions this note originally flagged.
...
**Does it actually win, though?** Checked directly against the current `evaluation_report.md`: `count_model` beats the best of the three §5 baselines on 7 of 37 `graduation_count` series, and beats Prophet specifically on 10 of 37. Both are real, positive evidence that the count-aware approach helps on *some* series — but neither number supports calling `graduation_count` "solved" by adding this algorithm. The honest framing: `count_model` is an evaluated alternative that measurably helps a minority of series, not a validated universal replacement for Prophet on this metric. Per this section's own "if a future measurement changes that picture, the honest response is to re-run the measurement" principle above, this number should be re-checked whenever the report is regenerated, not assumed to hold.
